...
struct door {
	...
	Map* nextMap;
	Sound* doorsound;
	door (int x, int y, int left, int top, int right, int bottom, SoundDevice* pSoundDevice, string filename/*Sound* sound*/) : nextMap_enter_position(x, y), prevMap_exit_triger_Rect{ left, top, right, bottom }, doorsound(new Sound(pSoundDevice, filename, 0)/*sound*/) {}
...
struct MAPSTRUCT {
	...
	float& volume;
	MAPSTRUCT(int id, string name, int tilesize, int step, int width, int height, string filepath, SoundDevice* pSoundDevice, string bgm, unordered_map<string, Sound*>* ShoeSoundpair, float& gvolume) : ID(id), NAME(name), TILESIZE(tilesize), STEP(step), WIDTH(width), HEIGHT(height), FILEPATH(filepath), pSoundDevice(pSoundDevice), BGMPATH(bgm), shoeSound(ShoeSoundpair), volume(gvolume) {}
};
class Map {
private:
	string name;
...
public:
	Map(int ID, string NAME, int TILESIZE, int STEP, int WIDTH, int HEIGHT, string filepath, SoundDevice* pSoundDevice, string bgm, unordered_map<string, Sound*>* shoeSoundpair, float& gVolume);
	~Map();
...
public:
	...
	bool AddMap(int ID, string NAME, int TILESIZE, int STEP, int WIDTH, int HEIGHT, string filepath, SoundDevice* pSoundDevice, string bgm, unordered_map<string, Sound*>* shoeSoundpair, float& volume);
